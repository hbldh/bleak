    ...
    class _Confirm(SupportsConfirm):
        async def confirm(self, device: BLEDevice, passkey: int) -> bool:
            if not central_passkey.done():
                central_passkey.set_result(passkey)
            return True

    async with BleakClient(device, pairing_callbacks=_Confirm(), pair=True) as client:
        assert client.is_connected

    assert await asyncio.wait_for(peripheral_passkey, 10) == await asyncio.wait_for(
        central_passkey, 10
    )
...
async def test_pair_passkey_entry_central_inputs(pairing_peripheral: Device) -> None:
    ...
    class _Request(SupportsRequestPasskey):
        async def request_passkey(self, device: BLEDevice) -> int | None:
            return await asyncio.wait_for(displayed, 10)

    async with BleakClient(
        device,
        pairing_callbacks=_Request(),
        pair=True,
    ) as client:
        assert client.is_connected
    ...
    class _Display(SupportsDisplayPasskey):
        async def display_passkey(self, device: BLEDevice, passkey: int) -> None:
            if not displayed.done():
                displayed.set_result(passkey)

    async with BleakClient(
        device,
        pairing_callbacks=_Display(),
        pair=True,
    ) as client:
        assert client.is_connected
