...
To run these tests, you need two Bluetooth controller. One that is connected to
your OS (e.g. the buildin BLE module of your PC/laptop) and controlled by
``bleak``. And one to create a virtual Bluetooth device that is controlled
via ``bumble``. These two need to be near to each other, so that they are in
receive range to each other.

The one to create a virtual Bluetooth devices can theoretically be any HCI supported
...
To run the integration tests you have to pass the ``hci-transport`` of your ``bumble`` 
device. You have to specify the bumble moniker of the transport. For more 
