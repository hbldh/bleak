...
class ObjcPeripheralDelegate(NSObject, protocols=[CBPeripheralDelegate]):
    ...
    py_delegate: weakref.ReferenceType["PeripheralDelegate"] = objc_py_property()
    ...
    def peripheralDidUpdateName_(  # pragma: no cover
        ...
        logger.debug(
            f"name of {get_prop(peripheral.identifier)} changed to {get_prop(peripheral.name)}"
        )
        # Currently not used in Bleak
    ...
    def peripheral_didModifyServices_(  # pragma: no cover
        ...
        logger.debug(
            f"{get_prop(peripheral.identifier)} invalidated services: {invalidatedServices}"
        )
        # Currently not used in Bleak
...
class PeripheralDelegate:
    ...
    def did_update_value_for_characteristic(
        ...
        if not error:
            ...
            if not future or (
                notification_discriminator
                and notification_discriminator(to_bytes(value))
            ):
                notify_callback = self._characteristic_notify_callbacks.get(c_handle)

                if notify_callback:
                    notify_callback(bytearray(to_bytes(value)))
                    return
        ...
        if error is not None:
            exception = (
                BleakGATTProtocolError(get_prop(error.code))
                if get_prop(error.domain) == CBATTErrorDomain
