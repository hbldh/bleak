...
class BleakAdapter:
    ...
    async def get_connected_devices(
        ...
        service_uuid_set = frozenset(normalize_uuid_str(u) for u in service_uuids)

        if not service_uuid_set:
            raise ValueError("service_uuids must not be empty")

        return await self._backend.get_connected_devices(service_uuid_set)
